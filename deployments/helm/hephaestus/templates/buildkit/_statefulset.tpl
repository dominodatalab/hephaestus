{{/*
Renders a full buildkit StatefulSet manifest (metadata, spec, volumeClaimTemplates), called once per
pool (implicit or explicit) from statefulset.yaml. The caller pre-resolves whichever fields differ
per pool (name/namespace/labels/serviceAccountName/checksum/replicaCount/resources/nodeSelector/
tolerations/affinity/priorityClassName/args/extra pod labels+annotations); everything else that
never varies per pool (image, rootless mode, mTLS, service port, persistence, etc.) is read directly
off ctx.root.

A full top-level document, so callers never nindent it - only its already-rendered sub-pieces
(labels) get nindented here, at the fixed depths every caller shares.

Expects a dict with:
  root                - $
  name                - resolved StatefulSet/Service/ServiceAccount/ConfigMap name
  namespace           - resolved namespace, or "" to omit metadata.namespace (implicit pool)
